import type { Config } from "tailwindcss";

export default {
  darkMode: ["class"],
  content: ["./pages/**/*.{ts,tsx}", "./components/**/*.{ts,tsx}", "./app/**/*.{ts,tsx}", "./src/**/*.{ts,tsx}"],
  prefix: "",
  theme: {
    container: {
      center: true,
      padding: "2rem",
      screens: {
        "2xl": "1400px",
      },
    },
    extend: {
      colors: {
        border: "hsl(var(--border))",
        input: "hsl(var(--input))",
        ring: "hsl(var(--ring))",
        background: "hsl(var(--background))",
        foreground: "hsl(var(--foreground))",
        primary: {
          DEFAULT: "hsl(var(--primary))",
          foreground: "hsl(var(--primary-foreground))",
        },
        secondary: {
          DEFAULT: "hsl(var(--secondary))",
          foreground: "hsl(var(--secondary-foreground))",
        },
        destructive: {
          DEFAULT: "hsl(var(--destructive))",
          foreground: "hsl(var(--destructive-foreground))",
        },
        muted: {
          DEFAULT: "hsl(var(--muted))",
          foreground: "hsl(var(--muted-foreground))",
        },
        accent: {
          DEFAULT: "hsl(var(--accent))",
          foreground: "hsl(var(--accent-foreground))",
        },
        popover: {
          DEFAULT: "hsl(var(--popover))",
          foreground: "hsl(var(--popover-foreground))",
        },
        card: {
          DEFAULT: "hsl(var(--card))",
          foreground: "hsl(var(--card-foreground))",
        },
        surface: "hsl(var(--surface))",
        "border-hover": "hsl(var(--border-hover))",
        "risk-low": "hsl(var(--risk-low))",
        "risk-low-bg": "hsl(var(--risk-low-bg))",
        "risk-low-text": "hsl(var(--risk-low-text))",
        "risk-medium": "hsl(var(--risk-medium))",
        "risk-medium-bg": "hsl(var(--risk-medium-bg))",
        "risk-medium-text": "hsl(var(--risk-medium-text))",
        "risk-high": "hsl(var(--risk-high))",
        "risk-high-bg": "hsl(var(--risk-high-bg))",
        "risk-high-text": "hsl(var(--risk-high-text))",
        "risk-critical": "hsl(var(--risk-critical))",
        "risk-critical-bg": "hsl(var(--risk-critical-bg))",
        "risk-critical-text": "hsl(var(--risk-critical-text))",
        "code-bg": "hsl(var(--code-bg))",
        "featured-border": "hsl(var(--featured-border))",
        "featured-badge-bg": "hsl(var(--featured-badge-bg))",
        "featured-badge-text": "hsl(var(--featured-badge-text))",
        "tag-bg": "hsl(var(--tag-bg))",
        "tag-text": "hsl(var(--tag-text))",
      },
      borderRadius: {
        lg: "var(--radius)",
        md: "calc(var(--radius) - 2px)",
        sm: "calc(var(--radius) - 4px)",
      },
    },
  },
  plugins: [require("tailwindcss-animate")],
} satisfies Config;
